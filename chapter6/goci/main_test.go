...
func setupGit(t *testing.T, proj string) func() {
	...
	var gitCmdList = []struct {
		...
		{[]string{"commit", "-m", "test"}, projPath, []string{
			"GIT_COMMITTER_NAME=test", "GIT_COMMITTER_EMAIL=test@example.com", "GIT_AUTHOR_NAME=test", "GIT_AUTHOR_EMAIL=test@example.com",
		}},
	}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expErr != nil {
				...
				if !errors.Is(err, tc.expErr) {
					t.Errorf("Exoected error: %q. Got %q.", tc.expErr, err)
				}
				return
