...
func tick(ctx context.Context, id int64, config *IntervalConfig, start, periodic, end Callback) error {
	...
	expire := time.After(i.PlannedDurarion - i.ActualDuration)
	start(i)
...
func newInterval(config *IntervalConfig) (Interval, error) {
	...
	case CategoryPomodoro:
		i.PlannedDurarion = config.PomodoroDuration
	case CategoryShortBreak:
		i.PlannedDurarion = config.ShortBreakDuration
	case CategoryLongBreak:
		i.PlannedDurarion = config.LongBreakDuration
	}

	i.ID, err = config.repo.Create(i)
	if err != nil {
		return i, err
	}
	return i, nil
