...
func newBarChart(ctx context.Context, config *pomodoro.IntervalConfig, update <-chan bool, erroCh chan<- error) (*barchart.BarChart, error) {
	bc, err := barchart.New(
		barchart.ShowValues(),
		barchart.BarColors([]cell.Color{
			cell.ColorBlue, cell.ColorYellow,
		}),
		barchart.ValueColors([]cell.Color{
			cell.ColorBlack, cell.ColorBlack,
		}),
	...
	updateWidget := func() error {
		...
		return bc.Values(
			[]int{
				int(ds[0].Minutes()),
				int(ds[1].Minutes())}, int(math.Max(ds[0].Minutes(), ds[1].Minutes())*1.1)+1,
		)
	}
