...
func newText(ctx context.Context, updateText <-chan string, errorChan chan<- error) (*text.Text, error) {
	txt, err := text.New()
	if err != nil {
		return nil, err
	}
...
func newDonut(ctx context.Context, donUpdater <-chan []int, errorChan chan<- error) (*donut.Donut, error) {
	don, err := donut.New(
		donut.Clockwise(),
	...
	go func() {
		for {
			...
			case d := <-donUpdater:
				if d[0] <= d[1] {
					errorChan <- don.Absolute(d[0], d[1])
				}
			case <-ctx.Done():
				return
			}
